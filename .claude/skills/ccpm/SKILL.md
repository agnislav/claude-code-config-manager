...
### 0. Capture — Lightweight idea parking lot
**When**: User wants to jot a thought without committing to a PRD ("add quick X", "capture X"), list pending ideas ("list quicks"), or cluster accumulated ideas into PRDs ("scope-quicks").
**Read**: `references/capture.md`
**Covers**: Appending one-line captures to `.claude/quicks.md`, listing them, and the LLM-driven scoping pass that clusters quicks into PRD seeds and archives resolved ones.
...
|---|---|
| Add a quick capture | `bash references/scripts/quick-add.sh "<text>"` |
| List open quicks | `bash references/scripts/quick-list.sh` |
...
```
Capture an idea:    "add quick <text>" / "capture <text>"
List quicks:        "list quicks" / "show my quicks"
Scope into PRDs:    "/ccpm scope-quicks" / "scope-quicks <theme>"
